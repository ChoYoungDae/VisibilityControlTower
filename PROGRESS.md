...
6. **Postgres 커넥션 재사용** — `query()`가 호출마다 새로 connect해서 재고
   ...
   개선.
7. **UI 폴리시 다수** (`fhd.html`, 사용자 스크린샷 피드백 반복 반영):
   - Overview 탭이 큰 화면(1600px+)에서 카드가 위로 쏠리고 하단에 빈
     공간이 많다는 피드백 — 처음엔 "Proactive Center" 소개 문구(문단 →
     3문단 불릿)를 추가해 채웠다가, 이후 사용자 요청으로 다시 삭제(카드
     레이아웃 자체는 유지: `#tab-overview .main`을 flex+`justify-content:
     center`로 세로 중앙 배치, 카드 패딩/간격 소폭 확대는 남김).
   - 비용 탭 월별 물류비 SVG 차트 — `max-width` 없이 `width:100%`라 넓은
     레이아웃에서 거의 2배로 늘어나 폰트·선이 다른 UI 대비 과도하게 커
     보이던 문제(원본 해상도에 가깝게 `max-width` 고정), 우측 TEU 축
     라벨이 플롯 영역 안쪽(막대·선과 가까운 자리)에 그려져 겹쳐 보이던
     문제(여백 확보 + 오른쪽 정렬로 수정).
   - 날짜·수량·괄호 라벨이 하이픈/공백 위치에서 어색하게 줄바꿈되던
     문제 다수(`2026-09-01`이 `2026-09-`/`01`로, `0 EA`가 숫자/단위로,
     `(CI/PL 확보)`·`Safety Stock 이하`가 괄호·음절 중간에서 끊기던 것,
     플로우 다이어그램 "관련 탭: ..." 줄 등) — 날짜·수량에 공통으로 쓰는
     `.num` 클래스에 `white-space:nowrap`을 추가해 시스템적으로 해결,
     나머지 개별 케이스는 nowrap span/`<br>`로 대응.
   - `.live-badge`(우상단 상태 배지) 겹침 버그 — 처음엔 `position:fixed`를
     유지한 채 겹치는 화면 폭 구간마다 상단 여백을 땜질로 늘렸으나
     (`@media (max-width:1350px)` 한정 → 폭 무관 전체 적용까지 두 차례
     수정) 스크롤 위치·화면 폭 조합에 따라 계속 다른 곳에서 겹침이
     재발함. 근본 원인이 "스크롤해도 화면에 고정"이라는 동작 자체라고
     판단해 최종적으로 `position:fixed` → `position:absolute`로 전환 —
     이제 배지는 페이지 콘텐츠와 함께 스크롤되어 다시는 겹치지 않음
     (겹침 방지용으로 넣었던 `.main` 상단 여백 임시 조치는 다시 제거).

**커밋·푸시 완료** — `41c9cd6`(Vercel/Supabase 배포 코드), `0145dc0`(Postgres
bool 캐스팅 버그 수정), `88267cd`(API_BASE 중복 경로 버그 수정), `75ae3b1`
(Postgres 커넥션 재사용), `83b64cd`~`9707906`(UI 폴리시 다수, 위 7번)까지
전부 `origin/master`에 반영됨. Vercel 프로젝트는 GitHub 연동이라 이후
push마다 자동 재배포된다. `DATABASE_URL`은 Vercel Project Settings의
환경변수로만 존재(레포에는 없음).
...
1. **재고/비용 탭 차트 인터랙티브화 + 폰트 버그 수정 (`mockup.html`)** —
   비용 탭 월별 물류비 차트에 재고 탭과 동일한 hover 툴팁·강조 인터랙션을
   추가. `.conf-trigger`(근거 보기 버튼) 등 폼 요소가 페이지 폰트를
   상속받지 않아 한글이 굴림으로 깨지던 버그를, 그 요소 하나만 고치는 대신
   `button,input,select,textarea{font-family:inherit}` 전역 규칙으로
   재발까지 막는 방식으로 수정.
2. **`fhd.html` 스케일/폰트 정합화** — `.app`의 zoom/font-size를 mockup과
   동일값(zoom 1.25 × 14px)으로 맞추고, 같은 원인(고정폭 폰트 스택이
   한글 글리프가 없어 generic monospace → 굴림으로 대체됨)의 `.num` 폰트
   버그를 이 파일에서 한 번 더 수정.
3. **Overview 카드 레이아웃 정리** — 2x2 그리드 유지로 결정(1x4로 펼치면
   카드가 좁아져 내부 지표가 줄바꿈되고 화면 하단이 텅 비는 걸 실측
   비교로 확인). 카드 상하 여백 확대, 재고 카드만 보라 강조선으로 성격이
   다름을 표시(화물/업무/비용은 중립 유지 — 넷을 전부 다른 색으로 주면
   "네 개가 같은 급으로 다르다"는 잘못된 신호가 된다는 사용자 판단으로
   되돌림). stat 박스를 높이 고정 + 상단 정렬로 바꿔서, 박스마다 콘텐츠
   양(trend-line 유무 등)이 달라 크기·글자 위치가 제각각으로 보이던
   문제를 구조적으로 해결.
4. **좌측 사이드바 접기/펼치기 토글 추가** — 기존에 좁은 화면에서만
   자동 발동되던 "아이콘만" 스타일을 버튼으로 수동 토글 가능하게 만들고
   `localStorage`로 상태 유지.
5. **상단 잡동사니 정리** — 브랜드 마크 아래 "Unified Visibility" 부제,
   모든 탭 상단 "검토 포인트" 안내 박스 제거. crit(지연) pill의 pulse
   애니메이션이 `fhd.html`에서만 `animation:none`으로 꺼져 있던 걸
   발견해 되살림(`mockup.html` 원본엔 있던 기능).
6. **헤더 라벨 정리 + 명칭 변경**: 사이드바/Overview 카드 제목에 "화물
   가시성/업무 가시성/비용 가시성/재고 예측"으로 명시적 이름 부여. 좌상단
   "LIVE OPERATIONS·시각" 표시를 우상단 고정 배지로 옮기고, 비운 좌상단
   자리엔 대시보드 이름 **"Proactive Center"**를 넣음(중간에 시도했던
   우상단 사용자 프로필 칩은 "별로"라는 피드백으로 제거). "Visibility
   Control Tower" 대신 이 이름을 쓰는 근거—가시성 자체가 아니라 그
   가시성이 가능하게 하는 더 정확한 예측·선제대응을 이름으로 강조—를
   `VisibilityControlTower_PRD.md` §6에 반영(v2.5). §3의 "Control Tower"
   내부 설계 개념 자체는 그대로 유지.

**커밋·푸시 완료** — 이 세션 변경분(`visibility_control_tower_fhd.html`
신설, `visibility_control_tower_mockup.html`/`VisibilityControlTower_
PRD.md`/이 파일 수정) 전부 `origin/master`에 반영됨. `raw data/`는
이번에도 공개 저장소 커밋 대상에서 제외(사용자 확인 필요 원칙 유지).

**다음 단계 (바뀌지 않음)** — 이번 세션은 `fhd.html` UI 다듬기라 이전에
정해둔 백로그 우선순위를 밀어내지 않는다. 파일 맨 아래 "## 다음 단계
(2026-08-10 세션 최신 — 위 §9.5 안내보다 우선)" 섹션이 여전히 유효한
다음 착수 지점이다(`api/README.md`/`db/README.md`에 남은 항목 우선,
§9.5는 그다음).

---

이전 갱신: 2026-08-10 (같은 날 이어진 긴 세션 — 목업을 저장할 **DB 구성**
논의로 시작해 **API 연동, 발주 추천 기능, vessel-tracker 버그 수정까지
전부 커밋·푸시(=이 프로젝트에서의 "배포")까지 마친** 세션. 진행 순서:

1. **synthetic-data 공백/drift 점검** — 화물/업무/비용 탭(Transportation
   도메인)은 synthetic-data가 아예 없었고, 재고 탭(Inventory 도메인)
   것도 목업이 여러 세션에 걸쳐 손으로 다듬어지며 CSV와 어긋나 있던
   (drift) 걸 확인해 전량 채움/재동기화.
2. **예측 모델 컨셉 검토** — 사용자가 첨부한 범용 AI Agent Platform
   마스터 프롬프트(가벼운 ML 스택)를 계기로, 우리 도메인에 맞는 예측
   후보를 검토(PTA 신뢰도 예측이 가장 적합, 재고 수요예측은 §9 책임
   경계상 범위 밖 등). 이어서 사용자 아이디어("재고 투영 Shortage 시점
   + 우리가 잘 아는 Inbound 리드타임을 결합해 발주 시점/수량 추천")를
   업무 범위 관점에서 검토해 승인 — "언제"·"얼마나"는 이미 우리가 갖고
   있거나 계산하는 값의 재조합이라 적절, "누구에게·PO 실행"은 계속
   범위 밖(§9/§12)이라는 원칙으로 정리.
3. **리드타임 이력 합성데이터 + SQLite DB(`db/`)** — 컨셉 계산 파이프라인
   구현(재고 투영 결정론적 계산 + 노선별 리드타임 P50/P95 통계 결합).
4. **API 연동(`api/`, FastAPI)** — 목업이 하드코딩 대신 이 DB를 실제로
   읽어 렌더링하도록 전환(화물/업무/비용 탭 detail 패널 + 재고 탭 전체).
   발주 추천 배너에 "오늘 발주하면 언제 도착하나(P50/P95)"까지 보강.
5. **vessel-tracker 버그 수정** — 선박 위치 추정 로직이 대권항로 직선
   보간만 써서 아시아→유럽 항로가 육지를 가로지르던 문제를 수에즈 운하
   등 실제 경유지 기반 다구간 경로로 수정.

**커밋·푸시 완료** — `1360520`(synthetic-data+DB+API+발주추천),
`153825d`(진행상황 문서화), `3a9a0e7`(vessel-tracker 수정), 전부
`origin/master`에 반영됨. `raw data/`는 이번에도 공개 저장소 커밋 대상에서
제외(사용자 확인 필요 원칙 유지). 상세 경과는 아래 세션별 기록 섹션 참고.
...
**커밋됨** — 이 시점 변경분은 아래 이어지는 세션들(DB/API 구현)과 함께
묶여서 커밋 `1360520`으로 커밋·`origin/master`에 푸시 완료(맨 아래 "커밋·
배포 완료" 참고).
...
**커밋됨** — 커밋 `1360520`으로 아래 이어지는 세션들과 함께 묶여 커밋·푸시
완료.
...
**커밋됨** — 커밋 `1360520`으로 아래 마지막 세션 변경분과 함께 묶여
커밋·푸시 완료.
...
사용자가 "여기까지 문서 정리하고, 배포해줘"로 명시 요청 — 이 프로젝트에서
"배포"는 별도 호스팅 파이프라인이 없어 **GitHub push**를 의미함(기존 관례).
이번 세션 전체 변경분(synthetic-data Transportation 도메인 + Inventory
drift 수정, `db/`, `api/`, 목업 API 연동, 발주 추천 도착 예상일 보강,
이 파일)을 커밋 `1360520`(master)으로 묶어 커밋하고 `origin/master`에
푸시 완료. 세션 시작 시점에 이미 있던 `VisibilityControlTower_PRD.md`
미커밋분(이전 세션 변경)도 같은 커밋에 함께 포함됨.

**`raw data/`는 이번에도 커밋 대상에서 제외**했다 — 공개(public) GitHub
저장소에 실제 고객 샘플 데이터를 올려도 되는지 사용자 확인이 아직 안
됐다는 기존 원칙([[raw_data_not_representative]] 참고) 그대로 유지.
`.gitignore`에도 아직 명시적으로 안 걸려 있으니, 다음에 이 폴더를 커밋할지
말지 다시 판단이 필요하면 그때 논의할 것.

## 2026-08-10 세션 (마지막, 계속) — vessel-tracker 추정 위치 버그 수정

사용자가 vessel-tracker 지도에서 선박이 "바다 위로 이동해야 하는데 직선으로
이동하거나 육지 위로 올라간다"고 지적. 원인 확인: `showMockPosition`(실
AIS 미연결 시 POL→POD 항로상에 대략 배치하는 폴백 로직)이 대권항로(구면
최단거리) 직선 보간만 쓰고 있었는데, 이건 부산→롱비치(태평양 횡단, 어차피
바다뿐)엔 문제없지만 **칭다오→로테르담/상하이→함부르크/닝보→앤트워프
(아시아→유럽) 항로는 대권항로가 러시아·중앙아시아를 그대로 가로질러
버리는 문제**였음 — 실제 배는 수에즈 운하를 거쳐 우회하는데 그 우회를
전혀 반영 못 하고 있었음.

**수정**: `vessel-tracker/public/index.html`에 `SUEZ_WAYPOINTS`(말라카
해협·아덴만·수에즈 운하 남/북단·지브롤터 해협 좌표)와 `ROUTE_WAYPOINTS`
매핑(위 3개 항로 전용) 추가, `pointAlongPath()`로 여러 구간의 대권항로를
실제 경유지 순서대로 이어서 보간하도록 재작성(`greatCirclePoint` 단일
호출 → 구간별 각거리 비례 배분). `showMockPosition`/`drawMockRoute` 둘 다
새 경유점 체인을 쓰도록 갱신. Node로 좌표 직접 계산해 칭다오→로테르담
항로가 싱가포르(100,2)→홍해(58,11)→지중해(25,33)→지브롤터(-5,36)를 거쳐
바다 위로만 지나가는 것 확인(육지 미통과). 부산↔롱비치·닝보↔부산처럼
원래도 바다만 지나던 항로는 매핑에 없어 기존 2점 대권항로 그대로 유지(영향
없음).

**결과물**: `vessel-tracker/public/index.html`.
