...
마지막 갱신: 2026-08-03 (업무 탭 1차 목업 작성 및 리뷰 진행 중)
...
- **명칭/톤**: 제목줄에 "Pantos View" **브랜드명**은 노출 안 함(브랜드 마크 CI만) — 단 "Visibility Control Tower"처럼 **컨셉/제품 성격을 나타내는 이름**은 이 규칙 대상이 아님, 브랜드 마크 옆에 텍스트로 노출하기로 확정(`cargo_tab_mockup.html`, `operation_tab_mockup.html` 상단 topnav에 반영 완료). 고객사명에 "귀하" 등 존칭 표현 사용 안 함
...
**업무 탭 구조 확정 → `operation_tab_mockup.html` 1차 목업 작성 완료, 현재 사용자와 화면 리뷰 진행 중.** PO 2단 리스트(PO/부킹), Cut-off 카운트다운, 스페이스 신호, 도착 준비(통관 게이트+Inland Routing 비교), Port-to-Port 스케줄 조회(2개 진입점), 요약 통계, 상단 타이틀까지 반영됨. 위 "업무 탭 — 논의 중인 콘텐츠 모델" 섹션과 "산출물 이력" 6번 참고.

**다음으로 할 것**:
1. `operation_tab_mockup.html` 리뷰 계속 — 화면/문구/인터랙션 피드백 반영.
2. 다 정리되면 → 화물+업무(+비용) 통합 파일로 합치는 작업 논의(다음 단계 결정 필요, 위 "산출물 이력" 참고).
