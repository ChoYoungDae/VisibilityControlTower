-- Visibility Control Tower — SQLite 스키마 (컨셉 데모)
-- synthetic-data/*.csv를 그대로 적재하기 위한 테이블 정의.
-- 목적: "재고 투영 + 리드타임 예측 -> 발주 시점/수량 추천" 컨셉을 실제 DB로
-- 보여주는 것. 프로덕션 스키마가 아니라 데모용 최소 구성이다.
--
-- 도메인 3개:
--   1) Inventory   (재고 탭 — PRD §9)
--   2) Transportation (화물/업무/비용 탭 — PRD §8)
--   3) Reorder Recommendation (신규 컨셉 — 이번 세션 합의)
...
CREATE TABLE item_shipment (
  shipment_row_id INTEGER PRIMARY KEY AUTOINCREMENT,
  item_id         TEXT NOT NULL REFERENCES item(item_id),
  container_no    TEXT,
  po_no           TEXT,
...
-- 계산 결과 (build_db.py가 재고 투영 + 리드타임 통계를 결합해 채움).
